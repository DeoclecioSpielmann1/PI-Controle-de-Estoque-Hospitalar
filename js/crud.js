...
class CrudRepository {
  ...
  _salvar(dados) { localStorage.setItem(this.storageKey, JSON.stringify(dados)); }

  criar(dados) {
    const erros = this._validar(dados);
    if (erros.length > 0) return { sucesso: false, erros };
    const registros = this._carregar();
    const novo = { id: Date.now(), ...this._aplicarDefaults(dados), criadoEm: new Date().toISOString(), atualizadoEm: new Date().toISOString() };
    registros.push(novo);
  ...
  listar(filtros = {}) {
    ...
    if (filtros.busca) {
      ...
      registros = registros.filter(r => Object.values(r).some(v => typeof v === 'string' && v.toLowerCase().includes(termo)));
    }
  ...
  buscarPorId(id) { return this._carregar().find(r => r.id === Number(id)) || null; }
  ...
  atualizar(id, dados) {
    ...
    registros[idx] = { ...registros[idx], ...dados, id: registros[idx].id, criadoEm: registros[idx].criadoEm, atualizadoEm: new Date().toISOString() };
    this._salvar(registros);
  ...
  _validar(dados, parcial = false) {
    ...
    this.schema.campos.forEach(campo => {
      ...
      if (dados[campo.nome] !== undefined && campo.tipo === 'number') {
        if (isNaN(Number(dados[campo.nome]))) erros.push(`O campo "${campo.label}" deve ser numérico.`);
      }
    });
    return erros;
  }
  ...
  _aplicarDefaults(dados) {
    const result = { ...dados };
    this.schema.campos.forEach(campo => {
      if (result[campo.nome] === undefined && campo.default !== undefined) result[campo.nome] = campo.default;
    });
    return result;
  }
